...
export function drawStaticLinks(ctx, snap, view, posOf) {
  ...
  snap.links.forEach(([a, b]) => {
    ...
    if (!pa || !pb) return;
    ctx.strokeStyle = "rgba(80,120,170,0.32)";
    ctx.beginPath();
...
export function drawFlows(ctx, snap, view, posOf, now) {
  snap.flows.forEach(([a, b, age]) => {
    ...
    ctx.strokeStyle = `rgba(16,110,200,${0.10 + 0.07 * k})`;
    ctx.lineWidth = 1.0;
    ctx.shadowColor = "rgba(16,110,200,0.35)";
    ctx.shadowBlur = 2;
    ctx.beginPath();
    ...
    ctx.fillStyle = "rgba(13,106,189,0.95)";
    ctx.shadowColor = "rgba(16,110,200,0.35)";
    ctx.shadowBlur = 2;
    ctx.beginPath();
...
export function drawFerries(ctx, snap, view, posOf, now) {
  snap.ferries.forEach(([a, b, age]) => {
    ...
    const x1 = pa[0] * view.scale + view.ox, y1 = pa[1] * view.scale + view.oy;
    const x2 = pb[0] * view.scale + view.ox, y2 = pb[1] * view.scale + view.oy;
    ctx.strokeStyle = `rgba(201,134,13,${0.35 + 0.3 * k})`;
    ctx.lineWidth = 2.0;
    ctx.setLineDash([6, 5]);
    ctx.shadowColor = "rgba(201,134,13,0.7)";
    ctx.shadowBlur = 6;
    ctx.beginPath();
    ctx.moveTo(x1, y1); ctx.lineTo(x2, y2);
    ctx.stroke();
...
export function drawPowerLinks(ctx, snap, view, posOf, now) {
  ...
  for (const [a, b, pw] of snap.power_links) {
    ...
    ctx.shadowColor = "rgba(224,72,72,0.3)"; ctx.shadowBlur = 2;
    ctx.beginPath();
    ctx.arc(ax + (bx - ax) * flow, ay + (by - ay) * flow, 1.8, 0, Math.PI * 2);
    ctx.fill();
    ctx.shadowBlur = 0;
    ctx.fillStyle = "rgba(186,58,58,0.5)";
