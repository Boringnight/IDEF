...
export function drawPipe(ctx, world, view, pipePath) {
  ...
  ctx.fillStyle = "rgba(199,215,231,0.65)";
  ctx.fill(pipePath);
  // 直接描边闭合轮廓(上下两壁都描到;原实现连续 beginPath 只描了下壁)
  ctx.strokeStyle = "rgba(78,110,148,0.5)";
  ctx.lineWidth = 1.5;
  ctx.stroke(pipePath);
  ...
  world.chambers.forEach((c, i) => {
    ...
    grd.addColorStop(0, `hsla(210, 45%, 42%, 0.06)`);
    grd.addColorStop(1, "transparent");
    ctx.fillStyle = grd;
    ctx.fillRect(cx - rx - 20, cy - ry - 20, rx * 2 + 40, ry * 2 + 40);
    ctx.fillStyle = `hsla(212, 45%, 36%, 0.66)`;
  ...
  world.throats.forEach(([a, b], i) => {
    ...
    ctx.strokeStyle = "rgba(90,115,150,0.32)";
    ctx.setLineDash([4, 4]);
    [a, b].forEach((x) => {
    ...
    ctx.setLineDash([]);
    ctx.fillStyle = "rgba(80,105,140,0.7)";
...
export function drawBoulders(ctx, boulders, view, dragIdx) {
  (boulders || []).forEach((b, i) => {
    ...
    ctx.fill();
    ctx.stroke();
    ctx.strokeStyle = "rgba(90,108,138,0.5)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ...
    ctx.stroke();
    if (dragIdx === i) {
      ctx.strokeStyle = "rgba(11,114,196,0.8)";
      ctx.setLineDash([4, 4]);
      ctx.beginPath();
...
export function drawDeployFog(ctx, snap, view, pipePath, W, H, samples) {
  ...
  grd.addColorStop(0, "rgba(146,160,180,0)");
  grd.addColorStop(1, "rgba(146,160,180,0.62)");
  ctx.fillStyle = grd;
  ...
  if (pct < 100 && samples) {
    ...
    ctx.fillStyle = "rgba(70,98,134,0.95)";
